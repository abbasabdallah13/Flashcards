...
const FlashCard = ({setAllowNewFlashcard, oldFlashcards, setOldFlashcards, setName, newCardId, setNewCardId, setFlashCards, flashCards}) => {
    ...
    useEffect(() => {
        ...
        client.fetch(query).then(data => {allSets.length===0 && setAllSets(data)}).catch(err => console.log(err));
        allSets.map(el => {
            if(el.setName === `${setName}`){
                setCurrentSetId(el._id)
            }
        })
